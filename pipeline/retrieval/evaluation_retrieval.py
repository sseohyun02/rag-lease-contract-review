...
def _recall_at_k(
    ...
    """법령/판례를 각각 독립적으로 상위 k개씩 잘라서 recall을 계산한다."""
...
def main() -> None:
    ...
    for k in RECALL_K_VALUES:
        for method in ("bm25", "dense", "rrf"):
            law_hits = sum(c["recall_at_k"][k][method]["law_hits"] for c in case_results)
            law_total = sum(c["recall_at_k"][k][method]["law_total"] for c in case_results)
            prec_hits = sum(c["recall_at_k"][k][method]["precedent_hits"] for c in case_results)
            prec_total = sum(c["recall_at_k"][k][method]["precedent_total"] for c in case_results)
            law_recall = round(law_hits / law_total, 4) if law_total else None
            prec_recall = round(prec_hits / prec_total, 4) if prec_total else None
            print(
                f"K={k:>2} [{method:>4}]  법령 recall={law_recall} ({law_hits}/{law_total})  "
                f"판례 recall={prec_recall} ({prec_hits}/{prec_total})"
            )
